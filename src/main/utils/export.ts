...
export async function exportToPDF(input: string, output: string) {
    const htmlPath = join(dirname(input), `${basename(input, '.md')}.html`);
    ...
    try {
        ...
        await htmlFileToPdf(htmlPath, output);
...
async function htmlFileToPdf(filePath: string, outputPdfPath: string) {
