...
import { availableDictionaries, loadSpellchecker, spellcheck, addToUserDictionary, removeFromUserDictionary, suggest, ignore } from './utils/spellchecker';
...
ipcMain.on('suggest', async (event, word: string) => {
...
ipcMain.handle('ignore', async (__dirname, word: string) => ignore(word));
